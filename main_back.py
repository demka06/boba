...
# ------------- [ DATA FOR CONN TO DB ] -----------
users = str(os.environ.get("SQL-USER"))
...
def checkPayForCitysAndVlgs( ):
	...
	while True:
		...
		for i in dfc:
			curs.execute(f"SELECT hps, plt FROM forts WHERE user_id = {i[0]}")
			fort = curs.fetchone( )
			curs.execute(f"SELECT {i[1]} - food FROM users WHERE user_id = {i[0]}")
			cr = curs.fetchone( )[0]
			b = 100 - fort[0]
			c = random.randint(1, 3)
			if b >= 10:
				curs.execute(
						f"UPDATE forts SET dfc = 1, last_check = {tim}, hps = hps - 10, plt = plt - plt/100*{c} WHERE user_id = {i[0]}"
						)
				conn.commit( )
			else:
				curs.execute(
						f"UPDATE forts SET dfc = 1, last_check = {tim}, plt = plt - plt/100*{c} WHERE user_id = {i[0]}"
						)
				conn.commit( )
			txt += f"\n@id{i[0]} не хватает {cr} ед. Продовольствия, чтобы прокормить жителей Форта. Погибло {c}% жителей форта.\n"
		vk.messages.send(
				peer_id=2e9 + 20,
				random_id=random.randint(0, 10000000000),
				message=txt
...
def checkAndPayForMilitary( ):
	while True:
		...
		for i in payers:
			now_utc = datetime.now(timezone('UTC'))
			times = int(now_utc.astimezone(timezone('Europe/Moscow')).timestamp( ))
			for c in army:
...
def updateInfoAboutForts( ):
	while True:
		...
		curs = conn.cursor( )
		curs.execute("SELECT user_id, vlg*5000+city*35000 FROM users")
		data = curs.fetchall( )
		for i in data:
			curs.execute(f"UPDATE plt = {i[1]} WHERE user_id = {i[0]}")
			conn.commit( )
		conn.close( )
		time.sleep(300)
...
th5 = threading.Thread(target=updateInfoAboutForts( ))
